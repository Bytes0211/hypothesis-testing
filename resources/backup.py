...
class Test:
    ...
    def validate_sample_data(self, info : dict = {}):
        validate = {}
        keyList = ['type', 'tail', 'replacement', 'N', 'sigma1', 'sigma2', 'n1', 'n2', 'xbar1', 'xbar2', 
                   'p', 'p_hat', 's1', 's2', 'cl', 'mar_err_min_n']
        for key in keyList:
            validate[key] = None 
    ...
    def calculate_test_statisics(self,):
        import numpy as np 
        import sys 
        sys.path.insert(0, "..")
        ...
        if self.type == "diff_of_mean":
            ...
            if self.sigma1 > 0 and self.sigma2 > 0: 
                ...
                self.critical_value = data.get_z_confidencd_level_critical_value(x = self.diff_of_means)
                ...
                if ((self.s1**2 / self.s2**2) > 2) or ((self.s2**2 / self.s1**2) > 2):
                    ...
                    if self.size == 'small':
                        df_frac1 = (self.s1**2 / self.n1)
                        df_frac2 = (self.s2**2 / self.n2)
                        nom = (df_frac1 + df_frac2)**2
                        denom = ((1 / (self.n1 - 1)) * (df_frac1)**2) + ((1 / (self.n2 - 1)) * (df_frac2)**2)   
                        self.df = nom / denom 
                else:
                    self.equal_variances = True 
                    self.pooled_variance = np.sqrt((((self.n1 - 1) * self.var1) * ((self.n2 - 1) * 
                                                            self.var2)/ (self.n1 + self.n2 - 2)))
